...
Linshape=(1/C.pi/wa)/(((pos-da)/wa)^2+1)*1E10;
Pa=2*C.h*C.co^2/lama^5/(exp(E/C.kB/Te)-1); %Planck body source function

ratio=L.E21/(4*C.pi)*n2*L.A21a*Linshape;
eps+=ratio;                                                          %bound-bound emission
eps+=1.63E-43*ne^2*lama^(-2)*Te^(-0.5)*L.g2a/Zi*(1-exp(-E/C.kB/Te)); %free-bound emisson
eps+=1.63E-43*ne^2*lama^(-2)*Te^(-0.5)*exp(-E/C.kB/Te);              %free-free emission
